...
- **Adding a connector `style` requires code, not config.** The eleven styles (`box`, `latch`, `grid`, `header-male`, `screw-terminal`, `barrier`, `button`, `xt30`, `sherlock`, `slide-switch`, `none` — `_BODY_STYLES` in `config.py`) are drawn by custom code. A new style must be implemented in **both** `pinout_gen/pinout_gen/renderer.py` and the designer's `pinout_design/js/svg-renderer.js`, or the two will disagree. `none` is the odd one out: both renderers return an empty string for it, and the page then leaves out the drawing slot and the type/pin-count line, so such a connector is a hotspot carrying only a name and a description.
